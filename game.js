...
function createConfetti() {
  confetti = [];
  const words = ["sort", "of", "confetti", "pickle", "yum"];
  for (let i = 0; i < 35; i++) {
    confetti.push({
...
function update() {
  ...
  pickles.forEach(p => {
    p.x -= scrollSpeed;
    p.bobOffset += 0.04;
    p.y = p.baseY + Math.sin(p.bobOffset) * 14;
  });
